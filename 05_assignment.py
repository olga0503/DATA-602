...
def exercise01():
    ...
    df = pd.DataFrame(np.array(range(0,12)).reshape(3,4))  

    #method 2
    #df = pd.DataFrame([[i for i in range(1,5)] for x in range(1,4)])
...
def exercise09():
    ...
    #no need to remove bottom two rows as they get removed by .loc['2017-09-1':'2018-10-05']
...
def exercise14(words):
    ...
    df = pd.DataFrame(words)[0].str.len()
...
def exercise15():
    ...
    df = exercise08()[0][['street','zip']].iloc[4::5]
