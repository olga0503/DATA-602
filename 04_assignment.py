...
class PandasChain():
    ...
    def display_chain(self): 
        #display commited blocks
        for block in self.__chain:
            block.display_transactions()
        #display ucommited blocks    
    ...
    def display_block_headers(self): 
        #display metadata of commited blocks
        for block in self.__chain:
            block.display_header()
        #display metadata of ucommited blocks    
...
class Block(PandasChain):
    ...
    def display_header(self): 
        print (self.__seq_id, self.__status, self.__block_hash, self.__prev_hash, self.__merkle_tx_hash, len(self.__transactions))
    ...
    def add_transaction(self,s,r,v):
        ...
        tx_hash = str(hashlib.sha256(str(str(ts) + str('s') + str('r') + str('v')).encode('utf-8')))
